...
float pid_i_gain_roll = 0.04; //0.04; //0.04;              //Gain setting for the roll I-controller
float pid_d_gain_roll = 15; //18.0; //18.0;              //Gain setting for the roll D-controller
...
float pid_p_gain_yaw = 3.0; //4.0; //4.0;                //Gain setting for the pitch P-controller. //4.0
float pid_i_gain_yaw = 0.02; //0.02; //0.02;               //Gain setting for the pitch I-controller. //0.02
...
int throttle, last_throttle, battery_voltage;
...
//  if(throttle <= 1500)throttle = 1000;       //for centred throttle
 

  if (start == 2){                                                          //The motors are started.
    if (throttle > 1800) throttle = 1800;
     if(throttle >= 1550) last_throttle++;
     if(throttle <= 1450) last_throttle--;
//    if(throttle <1010) last_throttle = 1010;
                                       //We need some room to keep full control at full throttle.
    if(last_throttle > 1800)last_throttle = 1800;
//    if(last_throttle < 1100)last_throttle = 1100;
    esc_1 = last_throttle - pid_output_pitch + pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 1 (front-right - CCW)
    esc_2 = last_throttle + pid_output_pitch + pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 2 (rear-right - CW)
    esc_3 = last_throttle + pid_output_pitch - pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 3 (rear-left - CCW)
    esc_4 = last_throttle - pid_output_pitch - pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 4 (front-left - CW)
